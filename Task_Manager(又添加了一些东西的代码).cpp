...
void read();
void save();
int choose(string a="无",string b="无",string c="无",string d="无",string e="上一页",string f="下一页");
void _return();
void find_task();
...
int choose(string a,string b,string c,string d,string e,string f){ 
	vector<string>all;
	all.push_back(e);
	all.push_back(a);
	all.push_back(b);
...
//	if(MODE==2)
	{
		...
		cout<<"----------------";
		gotoxy(50,2);cout<<"                          ";
		gotoxy(50,2);cout<<"1."<<e;
		
		gotoxy(50,3);
		cout<<"----------------";
		gotoxy(50,5);cout<<"                          ";
		gotoxy(50,5);cout<<"2."<<a;
		
		gotoxy(50,7);
		cout<<"----------------";
		gotoxy(50,9);cout<<"                          ";
		gotoxy(50,9);cout<<"3."<<b;
		
		gotoxy(50,11);
		cout<<"----------------";
		gotoxy(50,13);cout<<"                          ";
		gotoxy(50,13);cout<<"4."<<c;
		 
		gotoxy(50,15);
		cout<<"----------------";
		gotoxy(50,17);cout<<"                          ";
		gotoxy(50,17);cout<<"5."<<d;
		
		gotoxy(50,19);
		cout<<"----------------";
		gotoxy(50,20);cout<<"                          ";
		gotoxy(50,20);cout<<"6."<<f;
		
		gotoxy(50,21);
		cout<<"----------------";
		gotoxy(50,22);cout<<"b--返回 u--更新 f--搜索";
		gotoxy(50,23);cout<<"s--保存 x--保存并退出";
		gotoxy(50,24);cout<<"用数字1,2,3,4,5,6选择";
		gotoxy(50,25);cout<<"请输入你的回复";
	}
	/*
	if(MODE==1){//////////////////////////////////////////
		gotoxy(50,1);
		...
		cout<<"----------------";
		gotoxy(50,4);cout<<"                          ";
		gotoxy(50,4);cout<<"2."<<a;
		
		gotoxy(50,5);
		cout<<"----------------";
		gotoxy(50,6);cout<<"                          ";
		gotoxy(50,6);cout<<"3."<<b;
		
		gotoxy(50,7);
		cout<<"----------------";
		gotoxy(50,8);cout<<"                          ";
		gotoxy(50,8);cout<<"4."<<c;
		//////////////////////////////////////////////////////
		gotoxy(50,9);
		cout<<"----------------";
		gotoxy(50,10);cout<<"b--返回 u--更新 f--搜索";
		gotoxy(50,11);cout<<"s--保存 x--保存并退出";
		gotoxy(50,12);cout<<"用数字1,2,3,4,5,6选择";
		gotoxy(50,13);cout<<"请输入你的回复";
	}
	if(MODE==3){
		...
		cout<<"b--返回 u--更新 f--搜索"<<endl<<"s--保存 x--保存并退出"<<endl<<"用数字1,2,3,4,5,6选择"<<endl<<"请输入你的回复\n";
	} 
	*/
	char ans;
	while(true){
...
void show_one(int i,bool inall=1)
{
	if(inall)
	{
		cout<<i+1000<<" "<<his[i].name<<endl;
		if(his[i].belong){
			cout<<"属于 "<<folder[his[i].belong]<<" 文件夹\n"; 
		} 
		else cout<<"不属于任何文件夹\n";
		if(his[i].stime==-1){
			cout<<"error 101\n";
		...
		if(MODE==2) cout<<endl<<endl;
	}
	else{
		cout<<i+1000<<" "<<del[i].name<<endl;
		if(del[i].belong){
			cout<<"属于 "<<folder[del[i].belong]<<" 文件夹\n"; 
		} 
		else cout<<"不属于任何文件夹\n";
		if(del[i].stime==-1){
			cout<<"error 101\n";
...
IL void find_task(){
	...
	if(str[i]>160){
		string t;t.push_back(str[i]);t.push_back(str[i+1]); 
		pat.push_back(t);
		i++;
	} 
	else{
		string t;t.push_back(str[i]);
		pat.push_back(t);
	} 
	
	vector<string>txt;
	vector<mem>value;
	for(unsigned int i=0;i<his.size();i++)//枚举每一个名字 
	{
		value.push_back({0,i});
		txt.clear();
		for(unsigned int j=0;j<his[i].name.size();j++)//分割处理 
		if(his[i].name[j]>160){
			string t;t.push_back(his[i].name[j]);t.push_back(his[i].name[j]); 
			txt.push_back(t);
			i++;
		}
		else{
			string t;t.push_back(his[i].name[j]);
			pat.push_back(t);
		}
		//全字匹配 
...
void new_task(){
	...
	cout<<"是否放入文件夹?\n";
	switch(choose("放入已有文件夹","放入新建文件夹","暂不放入文件夹"))
	{
		case 1:{
...
{
	while(true){
		switch(choose("对任务排序","选中一项","随机","下一个排列"))
		{
			case 0:{
				return;
				break;
			...
			case 1:{
				cout<<"按照什么依据排序？(默认将已完成的任务排到后面)\n";
				switch(choose("起始时间","名称拼音","预设时间","进行翻转"))
				{
					case 0:{
						return;
						break;
...
void bug()
{
	if(system("dir /a C:\\task\\task.lib")==1){
		system("md C:\\task\\");
		system("copy D:\\task\\task.lib C:\\task\\");
		system("copy E:\\task\\task.lib C:\\task\\");
	}
	ifstream tin("C:\\task\\task.lib",ios::in);
	string x;tin>>x;
	if(x!=V())//标识符 
	{
		ifstream uin("C:\\task\\task.lib",ios::in);
		ofstream uout("C:\\task\\task.lib",ios::out);
		
		task t;
		int n;uin>>n;
		for(int i=0;i<n;i++){
			uin>>t.name;
			uin>>t.stime>>t.ftime>>t.rtime;
			his.push_back(t);
		}
		uin>>n;
		for(int i=0;i<n;i++){
